...
class Evaluation:
    ...
    """
    def __init__(self, fitness_functions, interval, 
                                        redundant=False, multiprocess=False):
        ...
        self.ff_idx = 0
        self.generational_count = 0

    @property
    def eval_count(self):
        """int : the number of evaluations that have been performed"""
        return self.fitness_functions[self.ff_idx].eval_count

    @eval_count.setter
    def eval_count(self, value):
        self.fitness_functions[self.ff_idx].eval_count = value
    ...
    def _serial_eval(self, population):
        for indv in population:
            if self._redundant or not indv.fit_set:
                indv.fitness = self.fitness_functions[self.ff_idx](indv)

        self.generational_count+=1
                ...
                if self._redundant or not indv.fit_set:
                    results.append(
                            pool.apply_async(_fitness_job,
                             (indv, self.fitness_functions[self.ff_idx], i)))
    ...
    def check_ff_idx(self):
        if self.generational_count >= self.interval:
            if self.ff_idx == len(self.fitness_functions)-1:
                self.ff_idx = 0
            else:
                self.ff_idx += 1
